...
import { t as require_browser } from "./browser-DPNvYonm.js";
import { a as oe$1, c as te$1, i as ne$1, l as y, n as T$1, o as p, r as a, s as se$1, t as R } from "./dist-D7FOtSlK.js";
...
var issueWarning$2;
var trustedTypes$1 = global$1.trustedTypes;
var emptyStringForBooleanAttribute$1 = trustedTypes$1 ? trustedTypes$1.emptyScript : "";
var polyfillSupport$2 = global$1.reactiveElementPolyfillSupportDevMode;
{
	const issuedWarnings = (_b$2 = global$1.litIssuedWarnings) !== null && _b$2 !== void 0 ? _b$2 : global$1.litIssuedWarnings = /* @__PURE__ */ new Set();
	...
	if (((_c$2 = global$1.ShadyDOM) === null || _c$2 === void 0 ? void 0 : _c$2.inUse) && polyfillSupport$2 === void 0) issueWarning$2("polyfill-support-missing", "Shadow DOM is being polyfilled via `ShadyDOM` but the `polyfill-support` module has not been loaded.");
	requestUpdateThenable = (name) => ({ then: (onfulfilled, _onrejected) => {
		...
		if (onfulfilled !== void 0) onfulfilled(false);
	} });
}
/**
...
var ReactiveElement = class extends HTMLElement {
	...
	static createProperty(name, options = defaultPropertyDeclaration) {
		...
		if (!options.noAccessor && !this.prototype.hasOwnProperty(name)) {
			...
			if (descriptor !== void 0) {
				...
				if (!this.hasOwnProperty("__reactivePropertyKeys")) this.__reactivePropertyKeys = new Set((_a = this.__reactivePropertyKeys) !== null && _a !== void 0 ? _a : []);
	...
	static finalize() {
		...
		this.elementStyles = this.finalizeStyles(this.styles);
		{
			const warnRemovedOrRenamed = (name, renamed = false) => {
				if (this.prototype.hasOwnProperty(name)) issueWarning$2(renamed ? "renamed-api" : "removed-api", `\`${name}\` is implemented on class ${this.name}. It has been ${renamed ? "renamed" : "removed"} in this version of LitElement.`);
			};
	...
	*/
	__initialize() {
		...
		this.requestUpdate();
		(_a = this.constructor._initializers) === null || _a === void 0 || _a.forEach((i) => i(this));
	}
	/**
	...
	*/
	addController(controller) {
		...
		if (this.renderRoot !== void 0 && this.isConnected) (_b = controller.hostConnected) === null || _b === void 0 || _b.call(controller);
	}
	/**
	...
	*/
	removeController(controller) {
		var _a;
		(_a = this.__controllers) === null || _a === void 0 || _a.splice(this.__controllers.indexOf(controller) >>> 0, 1);
	}
	/**
	...
	*/
	connectedCallback() {
		var _a;
		if (this.renderRoot === void 0) this.renderRoot = this.createRenderRoot();
		this.enableUpdating(true);
		(_a = this.__controllers) === null || _a === void 0 || _a.forEach((c) => {
			var _a;
	...
	*/
	...
	__propertyToAttribute(name, value, options = defaultPropertyDeclaration) {
		...
		if (attr !== void 0 && options.reflect === true) {
			const attrValue = (((_a = options.converter) === null || _a === void 0 ? void 0 : _a.toAttribute) !== void 0 ? options.converter : defaultConverter).toAttribute(value, options.type);
			if (this.constructor.enabledWarnings.indexOf("migration") >= 0 && attrValue === void 0) issueWarning$2("undefined-attribute-value", `The attribute value for the ${name} property is undefined on element ${this.localName}. The attribute will be removed, but in the previous version of \`ReactiveElement\`, the attribute would not have changed.`);
	...
	_$attributeToProperty(name, value) {
		...
		if (propName !== void 0 && this.__reflectingProperty !== propName) {
			...
			const converter = typeof options.converter === "function" ? { fromAttribute: options.converter } : ((_a = options.converter) === null || _a === void 0 ? void 0 : _a.fromAttribute) !== void 0 ? options.converter : defaultConverter;
	...
	requestUpdate(name, oldValue, options) {
		...
		if (name !== void 0) {
			...
			if ((options.hasChanged || notEqual)(this[name], oldValue)) {
				if (!this._$changedProperties.has(name)) this._$changedProperties.set(name, oldValue);
				if (options.reflect === true && this.__reflectingProperty !== name) {
					if (this.__reflectingProperties === void 0) this.__reflectingProperties = /* @__PURE__ */ new Map();
					this.__reflectingProperties.set(name, options);
				}
			} else shouldRequestUpdate = false;
		}
		if (!this.isUpdatePending && shouldRequestUpdate) this.__updatePromise = this.__enqueueUpdate();
	...
	*/
	performUpdate() {
		...
		debugLogEvent$1 === null || debugLogEvent$1 === void 0 || debugLogEvent$1({ kind: "update" });
		if (!this.hasUpdated) {
			const shadowedProperties = [];
			(_a = this.constructor.__reactivePropertyKeys) === null || _a === void 0 || _a.forEach((p) => {
				var _a;
				if (this.hasOwnProperty(p) && !((_a = this.__instanceProperties) === null || _a === void 0 ? void 0 : _a.has(p))) shadowedProperties.push(p);
			});
		...
		try {
			...
			if (shouldUpdate) {
				...
				(_b = this.__controllers) === null || _b === void 0 || _b.forEach((c) => {
					var _a;
	...
	*/
	...
	_$didUpdate(changedProperties) {
		...
		if (this.isUpdatePending && this.constructor.enabledWarnings.indexOf("change-in-update") >= 0) issueWarning$2("change-in-update", `Element ${this.localName} scheduled an update (generally because a property was set) after an update completed, causing a new update to be scheduled. This is inefficient and should be avoided unless the next update can only be scheduled as a side effect of the previous update.`);
	}
	__markUpdated() {
		this._$changedProperties = /* @__PURE__ */ new Map();
		this.isUpdatePending = false;
...
polyfillSupport$2 === null || polyfillSupport$2 === void 0 || polyfillSupport$2({ ReactiveElement });
{
	...
	ReactiveElement.enableWarning = function(warning) {
		...
		if (this.enabledWarnings.indexOf(warning) < 0) this.enabledWarnings.push(warning);
	};
...
if (global$1.reactiveElementVersions.length > 1) issueWarning$2("multiple-versions", "Multiple versions of Lit loaded. Loading multiple versions is not recommended.");
//#endregion
...
var issueWarning$1;
(_a$2 = global.litIssuedWarnings) !== null && _a$2 !== void 0 || (global.litIssuedWarnings = /* @__PURE__ */ new Set());
...
var wrap$1 = ((_b$1 = global.ShadyDOM) === null || _b$1 === void 0 ? void 0 : _b$1.inUse) && ((_c$1 = global.ShadyDOM) === null || _c$1 === void 0 ? void 0 : _c$1.noPatch) === true ? global.ShadyDOM.wrap : (node) => node;
...
var getTemplateHtml = (strings, type) => {
	...
	for (let i = 0; i < l; i++) {
		...
		html += regex === textEndRegex ? s + nodeMarker : attrNameEndIndex >= 0 ? (attrNames.push(attrName), s.slice(0, attrNameEndIndex) + boundAttributeSuffix + s.slice(attrNameEndIndex)) + marker + end : s + marker + (attrNameEndIndex === -2 ? (attrNames.push(void 0), i) : end);
	}
	return [trustFromTemplateString(strings, html + (strings[l] || "<?>") + (type === SVG_RESULT ? "</svg>" : "")), attrNames];
};
var Template = class Template {
	constructor({ strings, ["_$litType$"]: type }, options) {
		...
		while ((node = walker.nextNode()) !== null && parts.length < partCount) {
			if (node.nodeType === 1) {
				...
				if (node.hasAttributes()) {
					const attrsToRemove = [];
					for (const name of node.getAttributeNames()) if (name.endsWith(boundAttributeSuffix) || name.startsWith(marker)) {
						const realName = attrNames[attrNameIndex++];
						attrsToRemove.push(name);
						if (realName !== void 0) {
							const statics = node.getAttribute(realName.toLowerCase() + boundAttributeSuffix).split(marker);
		...
		debugLogEvent === null || debugLogEvent === void 0 || debugLogEvent({
			kind: "template prep",
			template: this,
...
function resolveDirective(part, value, parent = part, attributeIndex) {
	...
	if ((currentDirective === null || currentDirective === void 0 ? void 0 : currentDirective.constructor) !== nextDirectiveConstructor) {
		(_b = currentDirective === null || currentDirective === void 0 ? void 0 : currentDirective["_$notifyDirectiveConnectionChanged"]) === null || _b === void 0 || _b.call(currentDirective, false);
		if (nextDirectiveConstructor === void 0) currentDirective = void 0;
		else {
			currentDirective = new nextDirectiveConstructor(part);
			currentDirective._$initialize(part, parent, attributeIndex);
		}
		if (attributeIndex !== void 0) ((_c = (_d = parent).__directives) !== null && _c !== void 0 ? _c : _d.__directives = [])[attributeIndex] = currentDirective;
...
var ChildPart = class ChildPart {
	...
	*/
	get parentNode() {
		...
		if (parent !== void 0 && (parentNode === null || parentNode === void 0 ? void 0 : parentNode.nodeType) === 11) parentNode = parent.parentNode;
		return parentNode;
	}
	/**
		...
		if (this._$committedValue !== value) {
			...
			if (sanitizerFactoryInternal !== noopSanitizer) {
				...
				if (parentNodeName === "STYLE" || parentNodeName === "SCRIPT") {
					...
					if (parentNodeName === "STYLE") message = "Lit does not support binding inside style nodes. This is a security risk, as style injection attacks can exfiltrate data and spoof UIs. Consider instead using css`...` literals to compose styles, and make do dynamic styling with css custom properties, ::parts, <slot>s, and by mutating the DOM rather than stylesheets.";
	...
	*/
	_$clear(start = wrap$1(this._$startNode).nextSibling, from) {
		var _a;
		(_a = this._$notifyConnectionChanged) === null || _a === void 0 || _a.call(this, false, true, from);
	...
	*/
	setConnected(isConnected) {
		...
		if (this._$parent === void 0) {
			...
			(_a = this._$notifyConnectionChanged) === null || _a === void 0 || _a.call(this, isConnected);
	...
	_$setValue(value, directiveParent = this, valueIndex, noCommit) {
		...
		if (strings === void 0) {
			...
			for (i = 0; i < strings.length - 1; i++) {
				...
				else if (value !== nothing) value += (v !== null && v !== void 0 ? v : "") + strings[i + 1];
...
var BooleanAttributePart = class extends AttributePart {
	...
	_commitValue(value) {
		...
		if (value && value !== nothing) wrap$1(this.element).setAttribute(this.name, emptyStringForBooleanAttribute);
		else wrap$1(this.element).removeAttribute(this.name);
	}
};
var EventPart = class extends AttributePart {
	...
	handleEvent(event) {
		var _a, _b;
		if (typeof this._$committedValue === "function") this._$committedValue.call((_b = (_a = this.options) === null || _a === void 0 ? void 0 : _a.host) !== null && _b !== void 0 ? _b : this.element, event);
...
var polyfillSupport$1 = global.litHtmlPolyfillSupportDevMode;
polyfillSupport$1 === null || polyfillSupport$1 === void 0 || polyfillSupport$1(Template, ChildPart);
((_d = global.litHtmlVersions) !== null && _d !== void 0 ? _d : global.litHtmlVersions = []).push("2.8.0");
if (global.litHtmlVersions.length > 1) issueWarning$1("multiple-versions", "Multiple versions of Lit loaded. Loading multiple versions is not recommended.");
/**
...
var LitElement = class extends ReactiveElement {
	...
	*/
	createRenderRoot() {
		...
		(_a = (_b = this.renderOptions).renderBefore) !== null && _a !== void 0 || (_b.renderBefore = renderRoot.firstChild);
		return renderRoot;
	}
	/**
	* Updates the element. This method reflects property values to attributes
	...
	*/
	connectedCallback() {
		var _a;
		super.connectedCallback();
		(_a = this.__childPart) === null || _a === void 0 || _a.setConnected(true);
	}
	/**
	...
	*/
	disconnectedCallback() {
		var _a;
		super.disconnectedCallback();
		(_a = this.__childPart) === null || _a === void 0 || _a.setConnected(false);
	}
	/**
...
(_b = globalThis.litElementHydrateSupport) === null || _b === void 0 || _b.call(globalThis, { LitElement });
var polyfillSupport = globalThis.litElementPolyfillSupportDevMode;
polyfillSupport === null || polyfillSupport === void 0 || polyfillSupport({ LitElement });
LitElement["finalize"] = function() {
	if (!ReactiveElement.finalize.call(this)) return false;
...
if (globalThis.litElementVersions.length > 1) issueWarning("multiple-versions", "Multiple versions of Lit loaded. Loading multiple versions is not recommended.");
//#endregion
...
var standardProperty = (options, element) => {
	if (element.kind === "method" && element.descriptor && !("value" in element.descriptor)) return {
		...element,
		finisher(clazz) {
...
*/
var _a;
(_a = window.HTMLSlotElement) === null || _a === void 0 || _a.prototype.assignedElements;
//#endregion
...
var ClassMapDirective = class extends Directive {
	constructor(partInfo) {
		var _a;
		super(partInfo);
		if (partInfo.type !== PartType.ATTRIBUTE || partInfo.name !== "class" || ((_a = partInfo.strings) === null || _a === void 0 ? void 0 : _a.length) > 2) throw new Error("`classMap()` can only be used in the `class` attribute and must be the only part in the attribute.");
	}
	render(classInfo) {
	...
	update(part, [classInfo]) {
		...
		if (this._previousClasses === void 0) {
			...
			for (const name in classInfo) if (classInfo[name] && !((_a = this._staticClasses) === null || _a === void 0 ? void 0 : _a.has(name))) this._previousClasses.add(name);
...
//#endregion
//#region node_modules/@motionone/utils/dist/array.es.js
function addUniqueItem(array, item) {
	array.indexOf(item) === -1 && array.push(item);
}
//#endregion
//#region node_modules/@motionone/utils/dist/clamp.es.js
var clamp = (min, max, v) => Math.min(Math.max(v, min), max);
//#endregion
//#region node_modules/@motionone/utils/dist/defaults.es.js
var defaults = {
	duration: .3,
	delay: 0,
	endDelay: 0,
...
//#endregion
//#region node_modules/@motionone/utils/dist/is-number.es.js
var isNumber = (value) => typeof value === "number";
//#endregion
//#region node_modules/@motionone/utils/dist/is-easing-list.es.js
var isEasingList = (easing) => Array.isArray(easing) && !isNumber(easing[0]);
//#endregion
//#region node_modules/@motionone/utils/dist/wrap.es.js
var wrap = (min, max, v) => {
	const rangeSize = max - min;
	return ((v - min) % rangeSize + rangeSize) % rangeSize + min;
};
//#endregion
//#region node_modules/@motionone/utils/dist/easing.es.js
function getEasingForSegment(easing, i) {
	return isEasingList(easing) ? easing[wrap(0, easing.length, i)] : easing;
}
//#endregion
//#region node_modules/@motionone/utils/dist/mix.es.js
var mix = (min, max, progress) => -progress * min + progress * max + min;
//#endregion
//#region node_modules/@motionone/utils/dist/noop.es.js
var noop = () => {};
var noopReturn = (v) => v;
//#endregion
//#region node_modules/@motionone/utils/dist/progress.es.js
var progress = (min, max, value) => max - min === 0 ? 1 : (value - min) / (max - min);
//#endregion
//#region node_modules/@motionone/utils/dist/offset.es.js
...
//#endregion
//#region node_modules/@motionone/utils/dist/interpolate.es.js
function interpolate(output, input = defaultOffset(output.length), easing = noopReturn) {
	...
	*/
	...
	return (t) => {
		let i = 0;
		for (; i < length - 2; i++) if (t < input[i + 1]) break;
...
//#endregion
//#region node_modules/@motionone/utils/dist/is-cubic-bezier.es.js
var isCubicBezier = (easing) => Array.isArray(easing) && isNumber(easing[0]);
//#endregion
//#region node_modules/@motionone/utils/dist/is-easing-generator.es.js
var isEasingGenerator = (easing) => typeof easing === "object" && Boolean(easing.createAnimation);
//#endregion
//#region node_modules/@motionone/utils/dist/is-function.es.js
var isFunction = (value) => typeof value === "function";
//#endregion
//#region node_modules/@motionone/utils/dist/is-string.es.js
var isString = (value) => typeof value === "string";
//#endregion
//#region node_modules/@motionone/utils/dist/time.es.js
var time = {
	ms: (seconds) => seconds * 1e3,
	s: (milliseconds) => milliseconds / 1e3
};
//#endregion
//#region node_modules/@motionone/easing/dist/cubic-bezier.es.js
...
//#endregion
//#region node_modules/@motionone/easing/dist/steps.es.js
...
//#endregion
//#region node_modules/@motionone/animation/dist/utils/easing.es.js
var namedEasings = {
...
//#endregion
//#region node_modules/@motionone/animation/dist/Animation.es.js
var Animation = class {
	constructor(output, keyframes = [0, 1], { easing, duration: initialDuration = defaults.duration, delay = defaults.delay, endDelay = defaults.endDelay, repeat = defaults.repeat, offset, direction = "normal", autoplay = true } = {}) {
		...
		this.tick = (timestamp) => {
			...
			delay = delay;
			let t = 0;
			if (this.pauseTime !== void 0) t = this.pauseTime;
			...
			*/
			if (this.playState === "finished" && this.pauseTime === void 0) t = this.totalDuration;
			/**
			...
			*/
			...
			output(latest);
			if (this.pauseTime === void 0 && (this.playState === "finished" || t >= this.totalDuration + endDelay)) {
	...
	play() {
		...
		if (this.pauseTime !== void 0) this.startTime = now - this.pauseTime;
	...
	stop() {
		var _a;
		this.playState = "idle";
		if (this.frameRequestId !== void 0) cancelAnimationFrame(this.frameRequestId);
	...
	set currentTime(t) {
		if (this.pauseTime !== void 0 || this.rate === 0) this.pauseTime = t;
...
//#endregion
//#region node_modules/@motionone/types/dist/MotionValue.es.js
/**
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/data.es.js
...
function getMotionValue(motionValues, name) {
	if (!motionValues.has(name)) motionValues.set(name, new MotionValue());
	return motionValues.get(name);
}
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/transforms.es.js
/**
...
var buildTransformTemplate = (transforms) => transforms.sort(compareTransformOrder).reduce(transformListToString, "").trim();
var transformListToString = (template, name) => `${template} ${name}(var(${asTransformCssVar(name)}))`;
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/css-var.es.js
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/feature-detection.es.js
...
var featureTests = {
	...
	finished: () => Boolean(testAnimation({ opacity: [0, 1] }, { duration: .001 }).finished),
	linearEasing: () => {
		try {
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/easing.es.js
var resolution = .015;
...
var convertEasing = (easing, duration) => {
	if (isFunction(easing)) return supports.linearEasing() ? `linear(${generateLinearEasingPoints(easing, duration)})` : defaults.easing;
	else return isCubicBezier(easing) ? cubicBezierAsString(easing) : easing;
};
var cubicBezierAsString = ([a, b, c, d]) => `cubic-bezier(${a}, ${b}, ${c}, ${d})`;
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/keyframes.es.js
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/get-style-name.es.js
function getStyleName(key) {
	if (transformAlias[key]) key = transformAlias[key];
	return isTransform(key) ? asTransformCssVar(key) : key;
}
//#endregion
//#region node_modules/@motionone/dom/dist/animate/style.es.js
var style = {
	get: (element, name) => {
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/stop-animation.es.js
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/get-unit.es.js
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/animate-style.es.js
...
function animateStyle(element, key, keyframesDefinition, options = {}, AnimationPolyfill) {
	...
	*/
	return () => {
		...
		*/
		if (isCssVar(name)) {
			if (supports.cssRegisterProperty()) registerCssVariable(name);
		...
		*/
		if (valueIsTransform && !supports.linearEasing() && (isFunction(easing) || isEasingList(easing) && easing.some(isFunction))) canAnimateNatively = false;
		/**
		...
		*/
		if (canAnimateNatively) {
			...
			*/
			if (definition) keyframes = keyframes.map((value) => isNumber(value) ? definition.toDefaultUnit(value) : value);
			/**
			* If this browser doesn't support partial/implicit keyframes we need to
			* explicitly provide one.
			*/
			if (keyframes.length === 1 && (!supports.partialKeyframes() || isRecording)) keyframes.unshift(readInitialValue());
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/options.es.js
var getOptions = (options, key) => options[key] ? Object.assign(Object.assign({}, options), options[key]) : Object.assign({}, options);
//#endregion
//#region node_modules/@motionone/dom/dist/utils/resolve-elements.es.js
function resolveElements(elements, selectorCache) {
	var _a;
	if (typeof elements === "string") {
		if (selectorCache) {
			(_a = selectorCache[elements]) !== null && _a !== void 0 || (selectorCache[elements] = document.querySelectorAll(elements));
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/utils/controls.es.js
...
var controls = {
	get: (target, key) => {
		...
		switch (key) {
			case "duration": return target.duration;
			case "currentTime": return time.s((activeAnimation === null || activeAnimation === void 0 ? void 0 : activeAnimation[key]) || 0);
			case "playbackRate":
			case "playState": return activeAnimation === null || activeAnimation === void 0 ? void 0 : activeAnimation[key];
			case "finished":
				if (!target.finished) target.finished = Promise.all(target.animations.map(selectFinished)).catch(noop);
 ...
 /**
			...
			*/
			...
			default: return typeof (activeAnimation === null || activeAnimation === void 0 ? void 0 : activeAnimation[key]) === "undefined" ? void 0 : () => target.animations.forEach((animation) => animation[key]());
		}
	},
	set: (target, key, value) => {
		switch (key) {
			case "currentTime": value = time.ms(value);
			case "playbackRate":
...
//#endregion
//#region node_modules/@motionone/dom/dist/utils/stagger.es.js
function resolveOption(option, i, total) {
	return isFunction(option) ? option(i, total) : option;
}
//#endregion
//#region node_modules/@motionone/dom/dist/animate/create-animate.es.js
...
//#endregion
//#region node_modules/@motionone/dom/dist/animate/index.es.js
var animate$1 = createAnimate(Animation);
//#endregion
//#region node_modules/motion/dist/animate.es.js
...
function rt() {
	...
	}[(e = ne$1.state.themeMode) != null ? e : "dark"];
	return {
...
var h = {
	...
	setTheme() {
		const e = document.querySelector(":root"), { themeVariables: o } = ne$1.state;
		if (e) {
...
var fe = class extends LitElement {
	render() {
		return html`<div class="wcm-toolbar-placeholder"></div><div class="wcm-toolbar">${v.WALLET_CONNECT_LOGO} <button @click="${se$1.close}">${v.CROSS_ICON}</button></div>`;
	}
};
...
var S = class extends LitElement {
	...
	backBtnTemplate() {
		return html`<button class="wcm-back-btn" @click="${T$1.goBack}">${v.BACK_ICON}</button>`;
	}
	actionBtnTemplate() {
		return html`<button class="wcm-action-btn" @click="${this.onAction}">${this.actionIcon}</button>`;
	}
	render() {
		const e = { "wcm-border": this.border }, o = T$1.state.history.length > 1, r = this.title ? html`<wcm-text variant="big-bold">${this.title}</wcm-text>` : html`<slot></slot>`;
...
var c = {
	...
	handleMobileLinking(e) {
		const { walletConnectUri: o } = p.state, { mobile: r, name: a$1 } = e, t = r?.native, l = r?.universal;
		c.setRecentWallet(e);
		function i(s) {
			let $ = "";
			t ? $ = a.formatUniversalUrl(t, s, a$1) : l && ($ = a.formatNativeUrl(l, s, a$1)), a.openHref($, "_self");
		}
		o && i(o);
	},
	...
	getCachedRouterWalletPlatforms() {
		const { desktop: e, mobile: o } = a.getWalletRouterData(), r = Boolean(e?.native), a$2 = Boolean(e?.universal);
		return {
			isDesktop: r,
	...
	goToConnectingView(e) {
		T$1.setData({ Wallet: e });
		const o = a.isMobile(), { isDesktop: r, isWeb: a$3, isMobile: t } = c.getCachedRouterWalletPlatforms();
		o ? t ? T$1.push("MobileConnecting") : a$3 ? T$1.push("WebConnecting") : T$1.push("InstallWallet") : r ? T$1.push("DesktopConnecting") : a$3 ? T$1.push("WebConnecting") : t ? T$1.push("MobileQrcodeConnecting") : T$1.push("InstallWallet");
	}
};
...
var oe = class extends LitElement {
	constructor() {
		super(), this.view = T$1.state.view, this.prevView = T$1.state.view, this.unsubscribe = void 0, this.oldHeight = "0px", this.resizeObserver = void 0, this.unsubscribe = T$1.subscribe((e) => {
	...
	firstUpdated() {
		this.resizeObserver = new ResizeObserver(([e]) => {
			...
			this.oldHeight !== "0px" && animate(this.routerEl, { height: [this.oldHeight, o] }, { duration: .2 }), this.oldHeight = o;
	...
	viewTemplate() {
		switch (this.view) {
			case "ConnectWallet": return html`<wcm-connect-wallet-view></wcm-connect-wallet-view>`;
			case "DesktopConnecting": return html`<wcm-desktop-connecting-view></wcm-desktop-connecting-view>`;
			case "MobileConnecting": return html`<wcm-mobile-connecting-view></wcm-mobile-connecting-view>`;
			case "WebConnecting": return html`<wcm-web-connecting-view></wcm-web-connecting-view>`;
			case "MobileQrcodeConnecting": return html`<wcm-mobile-qr-connecting-view></wcm-mobile-qr-connecting-view>`;
			case "WalletExplorer": return html`<wcm-wallet-explorer-view></wcm-wallet-explorer-view>`;
			case "Qrcode": return html`<wcm-qrcode-view></wcm-qrcode-view>`;
			case "InstallWallet": return html`<wcm-install-wallet-view></wcm-install-wallet-view>`;
			default: return html`<div>Not Found</div>`;
		}
	}
	async onChangeRoute() {
		await animate(this.routerEl, {
			opacity: [1, 0],
			scale: [1, 1.02]
		}, {
			duration: .15,
			delay: .1
		}).finished, this.view = T$1.state.view, animate(this.routerEl, {
			opacity: [0, 1],
			scale: [.99, 1]
		}, {
			duration: .37,
...
var ne = class extends LitElement {
	constructor() {
		super(), this.open = !1, this.unsubscribe = void 0, this.timeout = void 0, this.unsubscribe = oe$1.subscribe((e) => {
	...
	render() {
		const { message: e, variant: o } = oe$1.state, r = {
...
function _t(e, o) {
	const r = Array.prototype.slice.call(import_browser.create(e, { errorCorrectionLevel: o }).modules.data, 0), a = Math.sqrt(r.length);
...
var Dt = { generate(e, o, r) {
	...
	i.forEach((y, u) => {
		y.forEach((O, b) => {
			if (i[u][b] && !(u < A && b < A || u > i.length - 8 && b < A || u < A && b > i.length - 8) && !(u > Ne && u < Ze && b > Ne && b < Ze)) {
...
var re = class extends LitElement {
	...
	render() {
		return html`<span><slot class="${classMap({
...
var L = class extends LitElement {
	...
	render() {
		var e;
		return html`<button @click="${this.handleClick.bind(this)}"><div><wcm-wallet-image walletId="${this.walletId}" imageId="${ifDefined(this.imageId)}"></wcm-wallet-image><wcm-text variant="xsmall-regular">${(e = this.label) != null ? e : c.getWalletName(this.name, !0)}</wcm-text>${this.sublabelTemplate()}</div></button>`;
	}
};
...
Q.styles = [h.globalCss, eo], se([property()], Q.prototype, "walletId", 2), se([property()], Q.prototype, "imageId", 2), se([property()], Q.prototype, "imageUrl", 2), Q = se([customElement("wcm-wallet-image")], Q);
...
var We = class extends LitElement {
	...
	async preloadListings() {
		if (y.state.enableExplorer) {
			await te$1.getRecomendedWallets(), p.setIsDataLoaded(!0);
			const { recomendedWallets: e } = te$1.state, o = e.map((r) => c.getWalletIcon(r));
...
var Ie = class extends LitElement {
	...
	render() {
		const { recomendedWallets: e } = te$1.state, o = [...e, ...e], r = a.RECOMMENDED_WALLET_AMOUNT * 2;
...
var D = class extends LitElement {
	...
	svgLoaderTemplate() {
		var e, o;
		const r = (o = (e = ne$1.state.themeVariables) == null ? void 0 : e["--wcm-wallet-icon-large-border-radius"]) != null ? o : h.getPreset("--wcm-wallet-icon-large-border-radius");
		let a = 0;
	...
	render() {
		...
		})}">${this.svgLoaderTemplate()}<wcm-wallet-image walletId="${ifDefined(this.walletId)}" imageId="${ifDefined(this.imageId)}"></wcm-wallet-image></div><wcm-text variant="medium-regular" color="${this.isError ? "error" : "primary"}">${this.isError ? "Connection declined" : this.label}</wcm-text>`;
	}
};
...
var G = {
	manualWallets() {
		var e, o;
		const { mobileWallets: r, desktopWallets: a$4 } = y.state, t = (e = G.recentWallet()) == null ? void 0 : e.id, i = (a.isMobile() ? r : a$4)?.filter((s) => t !== s.id);
		return (o = a.isMobile() ? i?.map(({ id: s, name: $, links: f }) => ({
			id: s,
			name: $,
			mobile: f,
			links: f
	...
	recomendedWallets(e = !1) {
		var o;
		const r = e || (o = G.recentWallet()) == null ? void 0 : o.id, { recomendedWallets: a } = te$1.state;
...
var Z = {
	...
	recentWalletTemplate() {
		...
		if (e) return html`<wcm-wallet-button name="${e.name}" walletId="${e.id}" imageId="${ifDefined(e.image_id)}" .recent="${!0}" .onClick="${() => this.onConnecting(e)}"></wcm-wallet-button>`;
	}
};
...
var Ee = class extends LitElement {
	render() {
		const { explorerExcludedWalletIds: e, enableExplorer: o } = y.state, r = e !== "ALL" && o, a = Z.manualWalletsTemplate(), t = Z.recomendedWalletsTemplate();
		let l = [
...
var ae = class extends LitElement {
	constructor() {
		super(), this.open = !1, this.active = !1, this.unsubscribeModal = void 0, this.abortController = void 0, this.unsubscribeModal = se$1.subscribe((e) => {
	...
	toggleBodyScroll(e) {
		if (document.querySelector("body")) if (e) document.getElementById("wcm-styles")?.remove();
	...
	onOpenModalEvent() {
		this.toggleBodyScroll(!1), this.addKeyboardEvents(), this.open = !0, setTimeout(async () => {
			const e = c.isMobileAnimation() ? { y: ["50vh", "0vh"] } : { scale: [.98, 1] }, o = .1, r = .2;
			await Promise.all([animate(this.overlayEl, { opacity: [0, 1] }, {
				delay: o,
				duration: r
	...
	async onCloseModalEvent() {
		...
		const e = c.isMobileAnimation() ? { y: ["0vh", "50vh"] } : { scale: [1, .98] }, o = .2;
		await Promise.all([animate(this.overlayEl, { opacity: [1, 0] }, { duration: o }).finished, animate(this.containerEl, e, { duration: o }).finished]), this.containerEl.removeAttribute("style"), this.active = !1, this.open = !1;
	}
	addKeyboardEvents() {
	...
	render() {
		return html`<wcm-explorer-context></wcm-explorer-context><wcm-theme-context></wcm-theme-context><div id="wcm-modal" class="${classMap({
			"wcm-overlay": !0,
			"wcm-active": this.active
		})}" @click="${this.onCloseModal}" role="alertdialog" aria-modal="true"><div class="wcm-container" tabindex="0">${this.open ? html`<wcm-modal-backcard></wcm-modal-backcard><div class="wcm-card"><wcm-modal-router></wcm-modal-router><wcm-modal-toast></wcm-modal-toast></div>` : null}</div></div>`;
	}
};
...
var Ae = class extends LitElement {
	...
	render() {
		const { recomendedWallets: e } = te$1.state, o = G.manualWallets(), r = [...e, ...o].reverse().slice(0, 4);
...
var me = class extends LitElement {
	...
	onFormatAndRedirect(e) {
		const { desktop: o, name: r } = a.getWalletRouterData(), a$5 = o?.native;
		if (a$5) {
			const t = a.formatNativeUrl(a$5, e, r);
			a.openHref(t, "_self");
		}
	}
	openDesktopApp() {
...
var Te = class extends LitElement {
	...
	render() {
		const { name: e, id: o, image_id: r, homepage: a$7 } = a.getWalletRouterData();
		return html`<wcm-modal-header title="${e}"></wcm-modal-header><wcm-modal-content><wcm-connector-waiting walletId="${o}" imageId="${ifDefined(r)}" label="Not Detected" .isStale="${!0}"></wcm-connector-waiting></wcm-modal-content><wcm-info-footer><wcm-text color="secondary" variant="small-thin">${`Download ${e} to continue. If multiple browser extensions are installed, disable non ${e} ones and try again`}</wcm-text><wcm-button .onClick="${() => this.onInstall(a$7)}" .iconLeft="${v.ARROW_DOWN_ICON}">Download</wcm-button></wcm-info-footer>`;
	}
};
...
var he = class extends LitElement {
	...
	onFormatAndRedirect(e, o = !1) {
		const { mobile: r, name: a$8 } = a.getWalletRouterData(), t = r?.native, l = r?.universal;
		if (t && !o) {
			const i = a.formatNativeUrl(t, e, a$8);
			a.openHref(i, "_self");
		} else if (l) {
			const i = a.formatUniversalUrl(l, e, a$8);
			a.openHref(i, "_self");
		}
	}
	openMobileApp(e = !1) {
	...
	render() {
		const { name: e, id: o, image_id: r, app: a$9, mobile: t } = a.getWalletRouterData(), { isWeb: l } = c.getCachedRouterWalletPlatforms(), i = a$9?.ios, s = t?.universal;
		return html`<wcm-modal-header title="${e}"></wcm-modal-header><wcm-modal-content><wcm-connector-waiting walletId="${o}" imageId="${ifDefined(r)}" label="Tap 'Open' to continue…" .isError="${this.isError}"></wcm-connector-waiting></wcm-modal-content><wcm-info-footer class="wcm-note"><wcm-platform-selection .isWeb="${l}" .isRetry="${!0}"><wcm-button .onClick="${() => this.openMobileApp(!1)}" .iconRight="${v.RETRY_ICON}">Retry</wcm-button></wcm-platform-selection>${s ? html`<wcm-text color="secondary" variant="small-thin">Still doesn't work? <span tabindex="0" @click="${() => this.openMobileApp(!0)}">Try this alternate link</span></wcm-text>` : null}</wcm-info-footer><wcm-info-footer class="wcm-app-store"><div><wcm-wallet-image walletId="${o}" imageId="${ifDefined(r)}"></wcm-wallet-image><wcm-text>${`Get ${e}`}</wcm-text></div><wcm-button .iconRight="${v.ARROW_RIGHT_ICON}" .onClick="${() => this.onGoToAppStore(i)}" variant="ghost">App Store</wcm-button></wcm-info-footer>`;
	}
};
...
var U = class extends LitElement {
	constructor() {
		super(...arguments), this.loading = !te$1.state.wallets.listings.length, this.firstFetch = !te$1.state.wallets.listings.length, this.search = "", this.endReached = !1, this.intersectionObserver = void 0, this.searchDebounce = c.debounce((e) => {
			e.length >= 1 ? (this.firstFetch = !0, this.endReached = !1, this.search = e, te$1.resetSearch(), this.fetchWallets()) : this.search && (this.search = "", this.endReached = this.isLastPage(), te$1.resetSearch());
		});
	}
	firstUpdated() {
	...
	isLastPage() {
		const { wallets: e, search: o } = te$1.state, { listings: r, total: a } = this.search ? o : e;
		return a <= De || r.length >= a;
	}
	async fetchWallets() {
		var e;
		const { wallets: o, search: r } = te$1.state, { listings: a$11, total: t, page: l } = this.search ? r : o;
		if (!this.endReached && (this.firstFetch || t > De && a$11.length < t)) try {
			this.loading = !0;
			const i = (e = p.state.chains) == null ? void 0 : e.join(","), { listings: s } = await te$1.getWallets({
	...
	render() {
		const { wallets: e, search: o } = te$1.state, { listings: r } = this.search ? o : e, a = this.loading && !r.length, t = this.search.length >= 3;
		...
		return html`<wcm-modal-header><wcm-search-input .onChange="${this.onSearchChange.bind(this)}"></wcm-search-input></wcm-modal-header><wcm-modal-content class="${classMap($)}"><div class="wcm-grid">${a ? null : l} ${a ? null : i} ${a ? null : r.map((f) => html`${f ? html`<wcm-wallet-button imageId="${f.image_id}" name="${f.name}" walletId="${f.id}" .onClick="${() => this.onConnect(f)}"></wcm-wallet-button>` : null}`)}</div><div class="wcm-placeholder-block">${s ? html`<wcm-text variant="big-bold" color="secondary">No results found</wcm-text>` : null} ${!s && this.loading ? html`<wcm-spinner></wcm-spinner>` : null}</div></wcm-modal-content>`;
	}
};
...
var we = class extends LitElement {
	...
	onFormatAndRedirect(e) {
		const { desktop: o, name: r } = a.getWalletRouterData(), a$12 = o?.universal;
		if (a$12) {
			const t = a.formatUniversalUrl(a$12, e, r);
			a.openHref(t, "_blank");
		}
	}
	openWebWallet() {
	...
	render() {
		const { name: e, id: o, image_id: r } = a.getWalletRouterData(), { isMobile: a$13, isDesktop: t } = c.getCachedRouterWalletPlatforms(), l = a.isMobile();
		return html`<wcm-modal-header title="${e}" .onAction="${c.handleUriCopy}" .actionIcon="${v.COPY_ICON}"></wcm-modal-header><wcm-modal-content><wcm-connector-waiting walletId="${o}" imageId="${ifDefined(r)}" label="${`Continue in ${e}...`}" .isError="${this.isError}"></wcm-connector-waiting></wcm-modal-content><wcm-info-footer><wcm-text color="secondary" variant="small-thin">${`${e} web app has opened in a new tab. Go there, accept the connection, and come back`}</wcm-text><wcm-platform-selection .isMobile="${a$13}" .isDesktop="${l ? !1 : t}" .isRetry="${!0}"><wcm-button .onClick="${this.openWebWallet.bind(this)}" .iconRight="${v.RETRY_ICON}">Retry</wcm-button></wcm-platform-selection></wcm-info-footer>`;
	}
};
